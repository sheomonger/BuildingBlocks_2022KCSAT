...
$x f(x)=2 x^{3}+a x^{2}+3 a+\int_{1}^{x} f(t) d t \cdots$ (ㄱ)
...
사인법칙에 의해 $2 R=\frac{\overline{\mathrm{BC}}}{\sin A}=\frac{\overline{\mathrm{BD}}}{\sin (\angle \mathrm{BCD})}$ 이므로
...
$\left|a_{m}\right|=\left|a_{m+3}\right|$ 에서 $a_{m+3}=\pm a_{m}$ 이다. $a_{m+3}=a_{m}$ 이면 $d=0$ 이므로 조건 (나)를 만족시 키지 못한다.
따라서 $a_{m+3}=-a_{m}$ 이므로
...
$f(x)=\left\{\begin{array}{cc}-2 x-2 & \left(-1 \leq x<-\frac{1}{2}\right) \\ 2 x & \left(-\frac{1}{2} \leq x \leq \frac{1}{2}\right) \\ -2 x+2 & \left(\frac{1}{2}<x \leq 1\right)\end{array}\right.$
...
$ \sum_{k=1}^{10}\left(a_{k}+2 b_{k}\right)=45 \quad \cdots \cdots$ (ㄱ)
