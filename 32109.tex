...
<<<S>>>01. $\frac{1}{\sqrt[4]{3}} \times 3^{-\frac{7}{4}}$
...
<<<S>>>03. 등비수열 $\left\{a_{n}\right\}$ 의 공비를 $r$ 라 하자.
...
<<<S>>>04.$ f(-1)=\lim _{x \rightarrow-1-} f(x)=-2+a$
...
$f^{\prime}(x)$는 $x=-2$, $x=1$에서 $0$이 되므로

$M=f(-2)=-16+12+24+1=21$이고
$m=f(1)=2+3-12+1=-6$
...
<<<S>>>06.$ \frac{\sin \theta}{1-\sin \theta}-\frac{\sin \theta}{1+\sin \theta}=\frac{2 \sin ^{2} \theta}{1-\sin ^{2} \theta}=4$
...
<<<S>>>09. 점 $\mathrm{P}$ 의 시각 $t$ 에서의 가속도를 $a(t)$ 라 하자.
...
<<<S>>>10. $y=a \sin b \pi x$ 의 주기는 $\frac{2 \pi}{b \pi}=\frac{2}{b}$ 이다.
따라서 점 $\mathrm{A}\left(\frac{1}{2 b}, a\right)$ 이고, 점 $\mathrm{B}\left(\frac{5}{2 b}, a\right)$ 이다.

(삼각형 $\mathrm{OAB}$ 의 넓이) $=\frac{1}{2} \times \frac{4}{2 b} \times a=\frac{a}{b}=5$
...
(ㄱ)에서 $a=5 b$ 이므로 $\frac{4}{5} \times(5 b)^{2} \times b^{2}=\frac{5}{4}$,
...
<<<S>>>11. $x f(x)=2 x^{3}+a x^{2}+3 a+\int_{1}^{x} f(t) d t \cdots$ (ㄱ)

(ㄱ)에 $x=1$ 을 대입하면 $f(1)=4 a+2 \quad \cdots$ (ㄴ)
...
따라서 $f(x)=3 x^{2}+2 a x+C$ (단, $C$ 는 상수 ) $\cdots \cdots$ (ㄷ)

(ㄷ)에 $x=1$ 을 대입하면 $f(1)=3+2 a+C$

(ㄴ)에 의해 $3+2 a+C=4 a+2$ 이다.
...
<<<S>>>12. 사인법칙에 의해 $2 R=\frac{\overline{\mathrm{BC}}}{\sin A}=\frac{\overline{\mathrm{BD}}}{\sin (\angle \mathrm{BCD})}$ 이므로

$\overline{\mathrm{BC}}=2 \times 2 \sqrt{7} \times \sin \frac{\pi}{3}=2 \sqrt{21}$
$\overline{\mathrm{BD}}=2 \times 2 \sqrt{7} \times \frac{2 \sqrt{7}}{7}=8$
$\sin (\angle \mathrm{BCD})=\frac{2 \sqrt{7}}{7} $ 이므로
$\cos (\angle \mathrm{BCD})=\frac{\sqrt{21}}{7}\left(\because \angle \mathrm{BCD}<\frac{\pi}{2}\right)$

$\overline{\mathrm{CD}}=x$ 로 놓고, 삼각형 $\mathrm{BCD}$ 에서 코사인법칙을 적용하면

$8^{2}=x^{2}+(2 \sqrt{21})^{2}-2 \times x \times 2 \sqrt{21} \times \frac{\sqrt{21}}{7}$
$x^{2}-12 x+20=0$
$x=2$ 또는 $x=10$
$\angle \mathrm{BDC}>\frac{\pi}{2}$ 이므로 $x=2$ 이다.
...
<<<S>>>13. $\left|a_{m}\right|=\left|a_{m+3}\right|$ 에서 $a_{m+3}=\pm a_{m}$ 이다. $a_{m+3}=a_{m}$ 이면 $d=0$ 이므로 조건 (나)를 만족시 키지 못한다.
따라서 $a_{m+3}=-a_{m}$ 이므로
...
$a_{1}=-45$ 이고 $a_{m+3}=-a_{m}$ 이므로 $a_{m+1}<0$ 이고 $a_{m+2}>0$ 이다.
...
$\sum_{k=1}^{m+1} a_{k}=\frac{(m+1)(-90+m d)}{2}>-100 \cdots $(ㄱ)

가능한 $(m, d)$ 의 순서쌍 중에서 (ㄱ)을 만족시키는 것은 $(1,30),(2,18)$ 뿐이다.
...
<<<S>>>14. 최고차항의 계수가 1 이고 $f^{\prime}(0)=f^{\prime}(2)=0$이므로 $f^{\prime}(x)=3 x(x-2)$ 이다.

그러므로 $f(x)=x^{3}-3 x^{2}+C$ (단, $C$ 는 상수 $)$

ㄱ. $p=1$ 일 때 $x>0$ 에서 $g(x)=f(x+1)-f(1)$ 이므로 $g^{\prime}(1)=f^{\prime}(2)=0$ (참)

ㄴ. $\lim _{x \rightarrow 0-} g(x)=\lim _{x \rightarrow 0-}\{f(x)-f(0)\}$
...
ㄷ. $g(x)= \begin{cases}x^{3}-3 x^{2} & (x \leq 0)\\ (x+p)^{3}-3(x+p)^{2}+C-\left(p^{3}-3 p^{2}+C\right) &(x>0)\end{cases}$
...
<<<S>>>15. $f(x)=\left\{\begin{array}{cc}-2 x-2 & \left(-1 \leq x<-\frac{1}{2}\right) \\ 2 x & \left(-\frac{1}{2} \leq x \leq \frac{1}{2}\right) \\ -2 x+2 & \left(\frac{1}{2}<x \leq 1\right)\end{array}\right.$
...
$a_{5}>0 $ 이면 $ f\left(a_{5}\right) \geq 0 $ 이므로 $a_{5}+f\left(a_{5}\right)>0 $ 이다.

$a_{5}<0 $ 이면 $f\left(a_{5}\right) \leq 0 $ 이므로 $a_{5}+f\left(a_{5}\right)<0 $ 이다.
$a_{5}=0 $ 이면 $f\left(a_{5}\right)=0 $ 이므로 $a_{5}+f\left(a_{5}\right)=0 $ 이다.
...
$a_{n+1}=0$ 이면 $f\left(a_{n}\right)=0$ 이므로 $a_{n}=0,1$ $a_{n+1}=1$ 이면 $f\left(a_{n}\right)=1$ 이므로 $a_{n}=\frac{1}{2}$ $a_{n+1}=\frac{1}{2}$ 이면 $f\left(a_{n}\right)=\frac{1}{2}$ 이므로 $a_{n}=\frac{1}{4}, \frac{3}{4}$
...
<<<S>>>16.$ \log _{2} 100-2 \log _{2} 5$ $=\log _{2} 100-\log _{2} 5^{2}$
...
<<<S>>>17. $f^{\prime}(x)=8 x^{3}-12 x^{2}+7$에서
...
<<<S>>>18. $ \sum_{k=1}^{10}\left(a_{k}+2 b_{k}\right)=45 \quad \cdots \cdots$ (ㄱ)

$\sum_{k=1}^{10}\left(a_{k}-b_{k}\right)=3 \cdots \cdots$ (ㄴ)

(ㄱ)-(ㄴ)이면

$\sum_{k=1}^{10} 3 b_{k}=42$

$\therefore \sum_{k=1}^{10} b_{k}=14$
...
$\frac{64-96+20}{4}=3 a^{2}-12 a+5 $ 이므로
$3 a^{2}-12 a+8=0$

근의 공식에 대입하면 $a=\frac{6 \pm \sqrt{12}}{3}$ 이므로 두 근
모두 $0<a<4$ 이다.
따라서 모든 실수 $a$ 의 값의 곱은 $\frac{8}{3}$ 이다.
$p=3, q=8 $ 이므로 $p+q=11 $ 이다.
...
$x>0 $ 이면 $f(x)+x>0 $ 이고, $x<0 $ 이면 $f(x)+x<0 $ 이다.

따라서 $|f(x)+x|= \begin{cases}f(x)+x & (x \geq 0) \\
-f(x)-x & (x<0)\end{cases}$

$\therefore x \geq 0 $ 일 때, $2 f(x)+x=6 x+k $
$x<0 $일 때, $-x=6 x+k$

정리하면

$\begin{cases}
x^{3}-9 x^{2}+15 x=k & (x \geq 0) \\
-7 x=k & (x<0)
\end{cases}$

이라 하고 그래프를 그리면 다음과 같다.

[IMG]

$y=g(x)$ 의 그래프와 직선 $y=k$ 가 서로 다른 4 개의 교점을 갖게 하는 정수 $k$ 의 값의 범위는 $0<k<7$
$\therefore 1 \leq k \leq 6$
...
<<<S>>>21. $y=\log _{a}(x-1)$ 의 역함수를 구해보면 $y=a^{x}+1$
...
따라서 점 $\mathrm{A}$ 를 $x$ 축의 방향으로 $-1$ 만큼, $y$ 축의 방향으로 1 만큼 평행이동한 점을 $\mathrm{A}^{\prime}$ 이라 하면 점 $\mathrm{A}^{\prime}$ 은 $y=a^{x}+1$ 의 그래프 위에 있다. 또한 $y=-x+4$ 의 기울기가 $-1$ 이므로 점 $\mathrm{A}^{\prime}$ 은 직선 $y=-x+4$ 위에 있다.
...
점 $\mathrm{P}$ 의 좌표는 $\left(\frac{1}{2}, \frac{7}{2}\right)$ 이다. 따라서 $a^{\frac{1}{2}}+1=\frac{7}{2}$ 에서 $a=\frac{25}{4}$ 이므로 점 $\mathrm{C}$ 의좌표는 $\left(0, \frac{4}{25}\right)$ 이다.
...
<<<S>>>22.$ \lim _{h \rightarrow 0+} \frac{|f(x+h)|-|f(x-h)|}{h}=h(x)$ 라 하자.
...
<<<S>>>23.$ \mathrm{E}(X)=60 \times \frac{1}{4}=15$
...
<<<S>>>24.$(a, b)$ 의 모든 순서쌍의 개수는 $4 \times 4=16$
...
<<<S>>>25.$\left(x^{2}+\frac{a}{x}\right)^{5}$ 의 전개식의 일반항은
...
<<<S>>>26. 주사위의 눈이 5 이상인 사건을 $X$, 꺼낸 공이 모 두 흰색인 사건을 $Y$ 라 하자.
...
<<<S>>>27. 확률변수 $X$ 의 표준편차를 $\sigma$ 라고 하면
확률변수 $X$ 는 정규분포 $\mathrm{N}\left(220, \sigma^{2}\right)$ 을 따르고,
확률변수 $\bar{X}$ 는 정규분포 $\mathrm{N}\left(220, \frac{\sigma^{2}}{n}\right)$ 을 따른다.
표준정규분포를 따르는 확률변수 $Z$ 에 대하여

$Z=\frac{\bar{X}-220}{\frac{\sigma}{\sqrt{n}}}$이므로

$\mathrm{P}(\bar{X} \leq 215) $
...
확률변수 $Y$ 의 표준편차는 $1.5 \sigma=\frac{3}{2} \sigma$ 이므로 확률변수 $Y$ 는 정규분포 $\mathrm{N}\left(240, \frac{9}{4} \sigma^{2}\right)$ 을 따르고,

확률변수 $\bar{Y}$ 는 정규분포 $\mathrm{N}\left(240, \frac{\sigma^{2}}{4 n}\right)$ 을 따른다. 표준정규분포를 따르는 확률변수 $Z$ 에 대하여

$Z=\frac{\bar{Y}-240}{\frac{\sigma}{2 \sqrt{n}}}$이므로

$\mathrm{P}(\bar{Y} \geq 235) $
...
지역 $\mathrm{A}$ 에 살고 있는 성인 중 임의추출한 $n$ 명의 $1$ 인 하루 물 사용량의 표본평균을 $\bar{X}$, 지역 $\mathrm{B}$ 에 살고 있는 성인 중 임의추출한 $9 n$ 명의 $1$ 인 하루 물 사용량의 표본평균을 $\bar{Y}$ 라 하자. $\mathrm{P}(\bar{X} \leq 215)=0.1587$ 일 때, $\mathrm{P}(\bar{Y} \geq 235)$ 의 값을 오른쪽 표준정규분포표를 이용하여 구한 것은? (단, 물 사용량의 단위는 $\mathrm{L}$이다.) [3점]
...
<<<S>>>28. 조건 (가에서 $f(3)+f(4)=5$ 또는 $10$ 이므로
...
<<<S>>>29. $\mathrm{E}(X)=1 \times a+3 \times b+5 \times c+7 \times b+9 \times a$
$=10 a+10 b+5 c$
...
<<<S>>>30.$\mathrm{A}, \mathrm{B}, \mathrm{C}, \mathrm{D}$ 학생이 받는 사인펜의 개수를 각각 $a, b, c, d$ 라 하면 $a+b+c+d=14$ 이고
...
$a^{\prime}+b^{\prime}+c^{\prime}+d^{\prime}=10$ 의 음이 아닌 정수해의 개수 는 ${ }_{4} \mathrm{H}_{10}=286$

$a^{\prime}, b^{\prime}, c^{\prime}, d^{\prime}$ 중 9 가 있는 경우의 수 ${ }_{4} \mathrm{C}_{1} \times{ }_{3} \mathrm{H}_{1}=4 \times 3=12$

$a^{\prime}, b^{\prime}, c^{\prime}, d^{\prime}$ 중 10 이 있는 경우의 수 ${ }_{4} \mathrm{C}_{1} \times{ }_{3} \mathrm{H}_{0}=4$
...
$a^{\prime \prime}+b^{\prime \prime}+c^{\prime \prime}+d^{\prime \prime}=5 $ 이므로${ }_{4} \mathrm{H}_{5}={ }_{8} \mathrm{C}_{5}=56$

이때, $a^{\prime}, b^{\prime}, c^{\prime}, d^{\prime}$ 중 10 이 있는 경우는 $a^{\prime}, b^{\prime}, c^{\prime}, d^{\prime}$ 이 모두 짝수인 경우에 포함되므로 문제의 경우의 수는 (ㄱ)-(ㄴ)-(ㄷ)-(ㄹ)+(ㄷ)$=218$


<<<P>>>30. 네 명의 학생 $\mathrm{A}, \mathrm{B}, \mathrm{C}, \mathrm{D}$ 에게 같은 종류의 사인펜 14 개를 다음 규칙에 따라 남김없이 나누어 주는 경우의 수를 구하시오.
[4점]
...
<<<S>>>24. $ 2 \cos \alpha =3 \sin \alpha$에서 
...
<<<S>>>25.$ \frac{d x}{d t}=e^{t}+4 e^{-t}, \frac{d y}{d t}=1$ 이므로
...
<<<S>>>27. $\overline{\mathrm{C}_{1} \mathrm{E}_{1}}=\sqrt{3}, \overline{\mathrm{C}_{1} \mathrm{~F}_{1}}=\frac{\sqrt{3}}{3}$ 이므로
...
$\therefore \frac{\frac{6-\sqrt{3}}{9}}{1-\frac{4-2 \sqrt{3}}{3}} $
...
<<<S>>>28.$\mathrm{A}^{\prime}(0,2)$ 라고 하자. $\frac{\pi}{6} \leq \theta \leq \frac{\pi}{3}$ 일 때 $1 \leq 2 \cos \theta \leq \sqrt{3}$ 이므로 점 Q는 선분 $\mathrm{OA}^{\prime}$ 위에 있고, 점 $\mathrm{R}$ 는 선분 $\mathrm{PB}$ 위 에 있다.
...
<<<S>>>29. $f(x)$ 의 최고차항의 계수가 양수이면 $x \rightarrow \pm \infty$ 일 때 $f(x) \rightarrow \infty$ 이므로 함수 $g(x)$ 는 최댓값을 갖지 않는다.
...
<<<S>>>30. 조건 (가)에서 (분모) $\rightarrow 0$ 일 때 극한이 존재하므로 (분자)$\rightarrow 0$이다.
...
<<<S>>>23. 점 $\mathrm{A}$ 를 $x y$ 평면에 대하여 대칭이동하면 $z$ 좌표의 부호만 반대로 하면 되므로 $\mathrm{B}(3,0,2), \mathrm{C}(0,4,2)$
...
<<<S>>>24.(점근선의 기울기 )$=\pm \frac{4}{a}$ 이므로 $\frac{4}{a}=3$
...
<<<S>>>25.$ \overrightarrow{p}=(x, y)$ 라 하고 원점을 $\mathrm{O}$ 라 하자.
...
<<<S>>>26. 문제의 조건에 따라 $\overline{\mathrm{AB}}=\overline{\mathrm{BF}}$ 이고 포물선의 정 의에서 $\overline{\mathrm{AB}}=\overline{\mathrm{AF}}$ 이므로 $\triangle \mathrm{ABF}$ 는 정삼각형이고 $\angle \mathrm{BFO}=60^{\circ}$ 이다.
...
<<<S>>>27.$ \overline{\mathrm{DP}}$ 는 점 $\mathrm{P}$ 가 점 $\mathrm{D}$ 에서 선분 $\mathrm{BC}$ 에 내린 수선 의 발 $\mathrm{H}$ 일 때 최소이고, $\overline{\mathrm{DB}} \times \overline{\mathrm{DC}}=\overline{\mathrm{BC}} \times \overline{\mathrm{DH}}$ 에서 $(\overline{\mathrm{DP}}$ 의 최솟값 $)=\overline{\mathrm{DH}}=\sqrt{3}$
...
<<<S>>>28.$ \mathrm{P}(2,3)$ 에서 접선의 방정식은 $\frac{2 x}{16}+\frac{3 y}{12}=1$ 에서 $y=-\frac{1}{2} x+4$ 이고 $x$ 축과의 교점 $\mathrm{S}$ 의 좌표는 $(8,0)$ 이다.
...
<<<S>>>29. 점 $\mathrm{G}$ 에서 $\overline{\mathrm{AB}}$ 에 내린 수선의 발을 $\mathrm{H}_{1}$ 이라 하면 삼수선의 정리에 의해 점 $\mathrm{P}$ 에서 $\overline{\mathrm{AB}}$ 에 내린 수선 의 발과 일치한다.
...
<<<S>>>30. $\overrightarrow{\mathrm{AP}} \cdot \overrightarrow{\mathrm{OC}} \geq \frac{\sqrt{2}}{2}$ 에서 $\overrightarrow{\mathrm{AP}}$ 와 $\overrightarrow{\mathrm{OC}}$ 가 이루는
각을 $\alpha$ 라 하면
